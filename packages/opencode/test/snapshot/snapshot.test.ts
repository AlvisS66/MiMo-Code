...
function run<A>(dir: string, body: (snapshot: Snapshot.Interface) => Effect.Effect<A, unknown>) {
...
test("diffFull fails on unknown anchors instead of returning empty statistics", async () => {
  await using tmp = await bootstrap()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      // Simulates a pruned snapshot anchor: git refuses the diff with
      // `fatal: bad object` (exit 128). The statistics path must surface the
      // failure — an empty result here would let callers clobber the last
      // good cached diff.
      const ghost = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef"
      const error = await run(tmp.path, (snapshot) =>
        snapshot.diffFull(ghost, before!, { patch: false }),
      ).then(
        () => undefined,
        (e) => e as { _tag?: string; step?: string; stderr?: string },
      )
      expect(error?._tag).toBe("SnapshotDiffFailedError")
      expect(error?.step).toBe("name-status")
      expect(error?.stderr).toContain("bad object")

      // Default (patch enabled) propagates the same failure.
      const errorPatched = await run(tmp.path, (snapshot) => snapshot.diffFull(ghost, before!)).then(
        () => undefined,
        (e) => e as { _tag?: string },
      )
      expect(errorPatched?._tag).toBe("SnapshotDiffFailedError")

      // diffFullPatched degrades to statistics-only instead of failing — its
      // consumers already treat empty results as "keep previous payload".
      const patched = await run(tmp.path, (snapshot) => snapshot.diffFullPatched(ghost, before!))
