...
// Raised when the git plumbing behind diff statistics fails (e.g. a pruned
// snapshot anchor: `fatal: bad object`, exit 128). Callers must treat this
// as "no data" — never as an empty diff — so cached results survive.
export class DiffFailedError extends Schema.TaggedErrorClass<DiffFailedError>()("SnapshotDiffFailedError", {
  from: Schema.String,
  to: Schema.String,
  step: Schema.String,
...
export interface Interface {
  ...
  ) => Effect.Effect<FileDiff[], DiffFailedError>
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Snapshot.state")(function* (ctx) {
        ...
        // A non-zero git exit (pruned anchor, corrupt snapshot repo) fails the
        // effect instead of masquerading as an empty diff.
        const rowsOf = Effect.fnUntraced(function* (from: string, to: string) {
          ...
          if (statuses.code !== 0) {
            return yield* Effect.fail(
              new DiffFailedError({ from, to, step: "name-status", stderr: statuses.stderr.trim() }),
            )
          }
          ...
          if (numstat.code !== 0) {
            return yield* Effect.fail(
              new DiffFailedError({ from, to, step: "numstat", stderr: numstat.stderr.trim() }),
            )
          }
        ...
        const diffFullPatched = Effect.fnUntraced(function* (from: string, to: string) {
          return yield* locked(
            Effect.gen(function* () {
              // Patch generation is a best-effort enrichment: a failing rows
              // lookup (e.g. pruned anchor) degrades to statistics-only here
              // so consumers keep their payload-trust / catch fallbacks.
              const rows = yield* rowsOf(from, to).pipe(
                Effect.catchTag("SnapshotDiffFailedError", (error) => {
                  log.warn("failed to list snapshot diff rows, returning statistics only", {
                    from,
                    to,
                    error,
                  })
